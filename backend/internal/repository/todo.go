...
func (r *TodoRepository) GetTodoByID(ctx context.Context, userID string, todoID uuid.UUID) (*todo.PopulatedTodo, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to execute create todo query for todo_id=%s user_id=%s: %w", todoID.String(), userID, err)
	}
...
func (r *TodoRepository) UpdateTodo(ctx context.Context, userID string, payload *todo.UpdateTodoPayload) (*todo.Todo, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to collect row form table:todos: %w", err)
	}
...
func (r *TodoRepository) GetTodoStats(ctx context.Context, userID string) (*todo.TodoStats, error) {
	...
	todoStats, err := pgx.CollectOneRow(rows, pgx.RowToStructByName[todo.TodoStats])
	if err != nil {
		return nil, fmt.Errorf("failed to collect row form table:todos: %w", err)
	}

	return &todoStats, nil
}
...
func (r *TodoRepository) DeleteAttachments(ctx context.Context, todoID uuid.UUID, attachmentID uuid.UUID) error {
	stmt := `
		DELETE FROM 
			todo_attachments
		WHERE
...
func (r *TodoRepository) UploadTodoAttachment(ctx context.Context, userID string, todoID uuid.UUID, name string, s3Key string, fileSize int64, mimeType string) (*todo.TodoAttachment, error) {
	stmt := `
		INSERT INTO
			todo_attachments
			(
	...
	if err != nil {
		return nil, fmt.Errorf("failed to execute upload todo attachment query for todo_id=%s: %w", todoID.String(), err)
	}

	todoAttachment, err := pgx.CollectOneRow(rows, pgx.RowToStructByName[todo.TodoAttachment])
	if err != nil {
		return nil, fmt.Errorf("failed to collect row from table:todo_attachments for todo_id=%s: %w", todoID.String(), err)
	}

	return &todoAttachment, nil
