...
type AWSConfig struct {
	AccesKeyID      string `koanf:"access_key_id" validate:"required"`
...
func LoadConfig() (*Config, error) {
	...
	err := k.Load(env.Provider("TASKER_", ".", func(s string) string { return strings.ToLower(strings.TrimPrefix(s, "TASKER_")) }), nil)
	if err != nil {
	...
	if err = k.Unmarshal("", mainConfig); err != nil {
		logger.Fatal().Err(err).Msg("cound not unmarshal main config")
	}
